...
func NewDefaultSnowflakeGenerator(workerID, dataCenterID int64) *Worker {
...
func NewSnowflakeGenerator(workerID, dataCenterID int64, lastTimestamp int64, sequence int64) *Worker {

	log.Debugf("Snoflake configuration [workerId: %d, datacenterId: %d]", workerID, dataCenterID)
	return &Worker{
		WorkerID:     workerID,
		LastStamp:    lastTimestamp,
		Sequence:     sequence,
		DataCenterID: dataCenterID,
	}
}
